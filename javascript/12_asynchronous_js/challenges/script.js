...
const whereAmI = function (latitude, longitude) {
  fetch(`https://geocode.maps.co/reverse?lat=${latitude}&lon=${longitude}`)
    .then((response) => {
      if (!response.ok)
        throw new Error(
          `Unable to find the location. Please provide the proper lat and lon value -- ${response.status}`
        );
...
reject the promise. If this part is too tricky for you, just watch the first
...
TEST DATA: 
Images in the img folder. Test the error handler by passing a wrong image path.
...
/*
let currentImg;
createImage("challenges/img/img-1.jpg")
  .then((img) => {
    currentImg = img;
  ...
  .then(() => {
    ...
    return createImage("challenges/img/img-2.jpg");
  })
  .then((img) => {
    currentImg = img;
  ...
  .then(() => {
    currentImg.style.display = "none";
    return createImage("challenges/img/img-3.jpg");
  })
  .then((img) => {
    currentImg = img;
...
more. Don't forget to test the error handler, and to set the network speed to 
...
const loadNPause = async function () {
  try {
    let currentImg = await createImage("challenges/img/img-1.jpg");
    await wait(2);
    currentImg.style.display = "none";

    currentImg = await createImage("challenges/img/img-2.jpg");
    await wait(2);
    currentImg.style.display = "none";

    currentImg = await createImage("challenges/img/img-3.jpg");
    await wait(2);
    currentImg.style.display = "none";
  } catch (error) {
    console.log(error);
